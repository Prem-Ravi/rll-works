...
export class WithdrawFundComponent implements OnInit {
  ...
  handleTransferFund(){
    ...
    if(Number(this.withdrawFundForm.get("amountToBeDeposited").value)>=500){ 
      if(Number(this.withdrawFundForm.get("amountToBeDeposited").value)>=5000000){
        alert("Minimum Amount can be Withdrawn: 5000000");
      }
      else
      {
      ...
      alert("Minimum Amount can be Withdrawn: 500")
    }
